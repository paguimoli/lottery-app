...
function assertCountsUnchanged(before, after) {
  for (const key of Object.keys(before)) {
    assert(before[key] === after[key], `${key} mutated during settlement evidence QA.`, {
      before,
      after,
    });
  }
...
pass("Settlement evidence optimization APIs require auth.");
...
const settlementTarget = baseline.scenarios.find(
  (scenario) =>
    scenario.scenario === "SETTLEMENT_PROCESSING" &&
    scenario.concurrency === 100
);

assert(settlementTarget, "Settlement evidence target missing.", { scenarios: baseline.scenarios });
assert(
  settlementTarget.measurementMode === "READ_ONLY_BASELINE" &&
    typeof settlementTarget.averageLatencyMs === "number" &&
    typeof settlementTarget.medianLatencyMs === "number" &&
    typeof settlementTarget.p95LatencyMs === "number" &&
    typeof settlementTarget.p99LatencyMs === "number" &&
    typeof settlementTarget.maxLatencyMs === "number" &&
    typeof settlementTarget.throughputPerSecond === "number" &&
    settlementTarget.resultCount > 0 &&
    Array.isArray(settlementTarget.stepMeasurements) &&
    settlementTarget.stepMeasurements.some((step) => step.step === "SETTLEMENT_EVIDENCE"),
  "Settlement evidence response contract changed or settlement evidence is not visible.",
  { settlementTarget }
);
assertAuthorityBaseline(authorityPayload.baselineStatus);

const reportResult = spawnSync("npm", ["run", "ops:settlement-evidence-optimization-report"], {
  encoding: "utf8",
  env: process.env,
});

if (reportResult.status !== 0) {
  fail("Settlement evidence optimization report failed.", {
...
try {
  ...
  fail("Settlement evidence optimization report did not return JSON.", {
...
assert(
  ...
  "No measured settlement evidence target improved.",
  { report }
);
assert(
  ...
  "Settlement evidence target regressed or lost evidence visibility.",
  { report }
);

pass("Settlement evidence optimization QA completed.", {
