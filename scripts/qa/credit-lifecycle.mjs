...
const unauthenticated = await requestJson("/api/authority/credit-lifecycle/summary", false);
assert(unauthenticated.response.status === 401, "Credit lifecycle should require auth.", {
  status: unauthenticated.response.status,
});
pass("Credit lifecycle endpoint requires auth.");

if (!sessionToken) fail("QA_ADMIN_SESSION_TOKEN is required.");

const [summaryResult, eventsResult] = await Promise.all([
  requestJson("/api/authority/credit-lifecycle/summary"),
  requestJson("/api/authority/credit-lifecycle/events"),
]);
...
assert(summaryResult.body.summary.domain === "CREDIT", "Credit lifecycle summary domain mismatch.", {
...
pass("Credit lifecycle evidence reporting is available.", {
