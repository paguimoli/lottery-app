...
const { response, body } = await requestJson(
  "/api/operations/idempotency-validation",
  {
    headers: { authorization: `Bearer ${sessionToken}` },
  }
);
...
if (!response.ok || body?.success !== true) {
  fail("Idempotency validation endpoint failed.", { status: response.status, body });
}

console.log(JSON.stringify(body.idempotencyValidation, null, 2));
