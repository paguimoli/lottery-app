...
const { response, body } = await requestJson("/api/operations/security-findings", {
  headers: { authorization: `Bearer ${sessionToken}` },
});

if (!response.ok || body?.success !== true) {
  fail("Security findings endpoint failed.", { status: response.status, body });
}

console.log(JSON.stringify(body.securityFindings, null, 2));
