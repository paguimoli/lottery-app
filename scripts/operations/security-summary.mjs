...
const { response, body } = await requestJson("/api/operations/security-summary", {
  headers: { authorization: `Bearer ${sessionToken}` },
});

if (!response.ok || body?.success !== true) {
  fail("Security summary endpoint failed.", { status: response.status, body });
}

console.log(JSON.stringify(body.securitySummary, null, 2));
